...
impl Problem {
    ...
    fn solve(&self, an: Vec<i64>) -> i64 {
        let (mut ans, mut max) = (0, an[0]);
        let mut is_positive = an[0] > 0;
        for a in an.into_iter().skip(1) {
            let is_not_negative = a > 0;
            if is_positive ^ is_not_negative {
                ans += max;
                max = a;
            } else {
                max = max.max(a);
