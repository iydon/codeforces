...
impl Problem {
    ...
    fn solve(&self, n: usize, x: u32, a: Vec<u32>) -> Option<usize> {
        if a.iter().all(|&a| a == 0) {
            return None;
        } else if a.iter().sum::<u32>() % x != 0 {
            return Some(n);
        } else {
            return Some(
                a.iter()
                    .rposition(|&a| a != 0)
                    .unwrap()
                    .max(n - a.iter().position(|&a| a != 0).unwrap() - 1),
            );
        }
    }
        ...
        O: std::io::Write,
    {
        ...
        for _ in 0..input.scalar::<u8>() {
            ...
            let a = input.vector::<u32>(n).into_iter().map(|a| a % x).collect();
            match self.solve(n, x, a) {
                Some(l) => writeln!(stdout, "{}", l).unwrap(),
